...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const paginationItems = document.querySelectorAll(".pagination a"); // Chọn tất cả các thẻ <a> trong pagination

    paginationItems.forEach(item => {
        if (item.getAttribute("href") === currentPage) {
            item.classList.add("active"); // Thêm class 'active' vào trang hiện tại
        } else {
            item.classList.remove("active"); // Xóa class 'active' khỏi các trang khác
        }
    });
});
